...
new #[Layout('layouts::app',['title' => 'Bank Transfer'])] class extends Component
{
    ...
    public $loadingTransaction = "";

    
    public $beneficiaries=[];

    public $showbanklist=false;
    public array $filteredBanks;
    public array $banks;
        ...
        // Log::info("banks",$response);
          ...
          if(isset($response['code']) && $response['code'] == '401'){
           ...
           if($response["status"] == true){
              ...
              $this->filteredBanks = $response['data'];
           }else{
    ...
    public function GetBeneficiaryBanks(){
        ...
        if($response['status'] == true){
                  $this->beneficiaries = $response['bankdata'] ?? [];
        }else{
    ...
    public function GetRecentTransactions(){
        ...
        $response = Http::withHeaders([
            "content-type" => "application/json",
            "Authorization" => "Bearer ".$this->token
        ])->get(config('services.api.base_url').'transactions/get-recent-transactions?transaction_type=bank');
    ...
    public function selectBank($code,$bname){
        ...
        //  $this->ValidateAccountNumber($this->accountNumber,$code,$this->transferType);

    }

     public function selectBeneficiary($bankcode, $bankname, $accountnumber){
    ...
    public function updatedbankname($value){
       ...
       $this->showbanklist = true;
        ...
        // Filter banks based on the input value
    $this->filteredBanks = array_values(array_filter($this->banks, function ($item) use ($value) {
                return str_contains(strtolower($item['bank_name']), strtolower($value));
            }));

    
    }

    public function selectRecentTransaction($code, $bname, $accountnumber){
    ...
    public function updatedAccountnumber()
    {
        ...
        if(strlen($this->accountnumber) == 10){
            ...
            $response = Http::withHeaders([
                "content-type" => "application/json",
                "Authorization" => "Bearer ".$this->token
            ])->post(config('services.api.base_url')."transactions/verify-bank-account",[
            ...
            if($response["status"] == true){
                ...
                $this->accountName = $response["data"]["first_name"];
            }else{
    ...
    public function InitiateTransfer(){
      ...
      $this->validate([
       ...
       $response = Http::withHeaders([
            "content-type" => "application/json",
            "Authorization" => "Bearer ".$this->token
       ])->post(config('services.api.base_url').'transactions/initiate-transaction',[
            "amount" => $this->amount,
    ...
    public function MakeTransfer(){
      ...
      $this->validate([
        ...
        $response = Http::withHeaders([
            "content-type" => "application/json",
            "Authorization" => "Bearer ".$this->token
       ])->post(config('services.api.base_url').'transactions/bank-transfer',[
            "amount" => $this->amount,
            ...
            <div class="md:hidden">
                 <div class="flex my-3 justify-center">
                    <flux:button href="#" variant="outline" >
                        ...
                        <flux:button href="{{ route('wallettransfer',['institution' => $institution_name]) }}" wire:navigate.hover variant="ghost">
                           To Wallet
                        </flux:button>
                 </div>
            </div>
    ...
    <div class="space-y-6">
               ...
               @if ($step == 1)
                ...
                {{-- <flux:input placeholder="Search Bank Name"  type="text" autocomplete='off' wire:model.live.throttle.100ms="bankname" />
                    ...
                    @if ($showbanklist)
                            <ul class="space-y-2">
                        @forelse ($filteredBanks as $bank)
                            <li class="p-2 bg-white shadow-md  dark:bg-gray-800 dark:hover:bg-white rounded dark:hover:text-gray-800 hover:text-white hover:bg-gray-800 cursor-pointer" wire:click="selectBank('{{$bank['bank_code']}}', '{{ucwords($bank['bank_name'])}}')">
         ...
         <div class="space-y-2">
                ...
                <div x-data="{ tab: 'recettrnx' }" class="w-full">
                        ...
                        <div class="mt-4">
                            <div x-show="tab === 'recettrnx'" wire:init='GetRecentTransactions'>
                                ...
                                @forelse ($recenttrnx as $item)
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <flux:text class="text-sm text-gray-500">{{ucwords($item['bankname'])}} - {{ucwords($item['accountNumber'])}}</flux:text>
                            ...
                            {{-- selectRecentTransaction('{{(int)$item['bankcode']}}', '{{ucwords($item['bankname'])}}','{{ $item['accountNumber'] }}') --}}
             ...
             <div class="bg-gray-100 dark:bg-gray-500 text-black dark:text-white p-2 rounded shadow border-gray-300 mb-3">
                  <div class="flex">
                    ...
                    <div class="flex items-center space-x-2 mt-2">
                        ...
                        <flux:text class="font-normal"> {{ ucwords($bankname) }}</flux:text>
                    </div>
                      </div>
                  </div>
             </div>
          ...
          <flux:heading size="lg">
                ...
                <div class="space-y-6 rounded">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <flux:text class="font-semibold"> {{ ucwords($bankname) }}</flux:text>
                    </div> 
                    <div class="flex justify-between items-center">
                    ...
                    {{-- <div class="flex justify-between items-center p-1">
                        <flux:text class="text-gray-500">Narration</flux:text>
                        <flux:text class="font-semibold">{{ $narration }}</flux:text>
                    </div> --}}

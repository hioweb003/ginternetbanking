...
<div>
     <?php ob_start(); ?><h1 class="font-medium [:where(&amp;)]:text-zinc-800 [:where(&amp;)]:dark:text-white text-2xl [&amp;:has(+[data-flux-subheading])]:mb-2 [[data-flux-subheading]+&amp;]:mt-2" data-flux-heading><?php ob_start(); ?>Manage Institution<?php echo trim(ob_get_clean()); ?></h1>

        <?php echo ltrim(ob_get_clean()); ?>

        

        <?php ob_start(); ?><div data-orientation="horizontal" role="none" class="border-0 [print-color-adjust:exact] bg-zinc-800/5 dark:bg-white/10 h-px w-full mb-10" data-flux-separator></div>
<?php echo ltrim(ob_get_clean()); ?>
...
<?php echo ltrim(ob_get_clean()); ?>
                        <?php ob_start(); ?><td class="py-3 px-3 first:ps-0 last:pe-0 text-sm  text-zinc-500 dark:text-zinc-300  not-in-[tr:first-child]:border-t border-zinc-800/10 dark:border-white/20 max-md:hidden" data-flux-cell>
    <?php ob_start(); ?><?php echo e(ucwords($row->fullname)); ?><?php echo trim(ob_get_clean()); ?>
...
<?php echo ltrim(ob_get_clean()); ?>
                       ...
                       <?php ob_start(); ?><td class="py-3 px-3 first:ps-0 last:pe-0 text-sm  text-zinc-500 dark:text-zinc-300  not-in-[tr:first-child]:border-t border-zinc-800/10 dark:border-white/20 max-md:hidden" data-flux-cell>
    <?php ob_start(); ?>
                             <?php ob_start(); ?><p class="[:where(&amp;)]:font-normal [:where(&amp;)]:text-sm [:where(&amp;)]:text-zinc-500 [:where(&amp;)]:dark:text-white/70 <?php echo e($row->status == 1 ?  'text-green-500' : 'text-red-500'); ?>" data-flux-text ><?php ob_start(); ?><?php echo e($row->status == 1 ?  'Active' : 'Inactive'); ?><?php echo trim(ob_get_clean()); ?></p><?php echo ltrim(ob_get_clean()); ?>
                          <?php echo trim(ob_get_clean()); ?>
...
<?php $__blaze->pushData(['paginator' => $this->GetInstitution]); ?>
<?php _b8c998151a2f73dcdcb98b150b950e54($__blaze, ['paginator' => $this->GetInstitution], [], ['paginator'], [], $__this ?? (isset($this) ? $this : null)); ?>
<?php $__blaze->popData(); ?>
</div><?php /**PATH /home/henry/Desktop/htdocs/grubinternetbanking/storage/framework/views/livewire/views/f021b5c9.blade.php ENDPATH**/ ?>